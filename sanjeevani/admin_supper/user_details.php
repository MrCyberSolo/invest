...
$row_user= mysqli_fetch_array($run);   
  ...
  $query_income = mysqli_fetch_array(mysqli_query($con, "select * from income where userid='$user_id'"));  
  	$today_bal = $query_income['today_bal'];
	$fran_bal = $query_income['fran_bal'];
	$current_bal = $query_income['current_bal'];
...
<body>
				...
				<div class="page-content">
						...
						<div class="row g-0 row-group">
							<div class="col-12 col-lg-3">
								<div class="card-body">
									<div class="d-flex align-items-center">
										<div class="">
											<h4 class="mb-0 font-weight-bold text-info"><?php echo $current_bal; ?></h4>
											<p class="mb-0">Current Balance</p>
										</div>
										<div class="font-40 ms-auto"><i class="lni lni-rupee"></i>
										</div>
									</div>
								</div>
							</div>
								...
								<div class="card-body">
									<div class="d-flex align-items-center">
										<div class="media-body">
											<h4 class="mb-0 font-weight-bold text-facebook"><?php echo $fran_bal; ?></h4>
											<p class="mb-0">Recharge Balance</p>
										</div>
										<div class="font-40 ms-auto"><i class="lni lni-rupee"></i>
										</div>
									</div>
							...
							<div class="col-12 col-lg-3">
								<div class="card-body">
									<div class="d-flex align-items-center">
										<div class="media-body">
											<h4 class="mb-0 font-weight-bold text-youtube">
											    <?php 
											        $int_amount = 0;
                									$query = mysqli_query($con,"select * from interest where int_userid='$user_id'order by int_id desc");
							...
							<div class="col-12 col-lg-3">
								<div class="card-body">
									<div class="d-flex align-items-center">
										<div class="media-body">
											<h4 class="mb-0 font-weight-bold text-linkedin">
											     <?php 
											        $int_amount = 0;
                									$query = mysqli_query($con,"select * from interest_label where int_userid='$user_id'order by int_id desc");
                									if(mysqli_num_rows($query)>0)
                									{
                										while($row=mysqli_fetch_array($query))
                										{
						...
						<div class="row g-0 row-group">
									...
									<div class="d-flex align-items-center">
										<div class="">
											...
											<?php 
                            					$i=1;
                            					$total_order =0;
                            					$query = mysqli_query($con,"select * from order_book where o_user_type='User' AND o_userid ='$user_id' order by o_id desc");
                            					if(mysqli_num_rows($query)>0){
                            						while($row=mysqli_fetch_array($query)){
							...
							<div class="col-12 col-lg-3">
								<div class="card-body">
									<div class="d-flex align-items-center">
										<div class="media-body">
    											<h4 class="mb-0 font-weight-bold text-facebook"><?php 
                                					$i=1;
                                					$total_withdrawl =0;
                                					$query = mysqli_query($con,"select * from income_received  where userid='$user_id'  order by id desc");
                                					if(mysqli_num_rows($query)>0){
                                						while($row=mysqli_fetch_array($query)){
                            								$amount = $row['amount'];
                            							    $total_withdrawl = $total_withdrawl + $amount;
                            						}}
                            						echo $total_withdrawl;
                            					?></h4>
											<p class="mb-0">Withdrawl</p>
										</div>
										<div class="font-40 ms-auto"><i class="lni lni-rupee"></i>
										</div>
									</div>
									...
									<div class="d-flex align-items-center">
										<div class="media-body">
											<h4 class="mb-0 font-weight-bold text-youtube">
											    <?php 
											        $payment_request = 0;
                									$query = mysqli_query($con,"select * from payment where user_id='$user_id' AND status='Accept' order by id desc");
                									if(mysqli_num_rows($query)>0)
                									{
                										while($row=mysqli_fetch_array($query))
                										{
                											$amount = $row['amount'];
                											$payment_request = $payment_request + $amount;
                										}
                									}
                									echo $payment_request;
											    ?>
											</h4>
											<p class="mb-0">Fund Add</p>
										</div>
										<div class="font-40 ms-auto"><i class="lni lni-rupee"></i>
										</div>
									</div>
									...
									<div class="d-flex align-items-center">
										<div class="media-body">
											<h4 class="mb-0 font-weight-bold text-linkedin">
											     <?php 
											        $int_amount = 0;
                									$query = mysqli_query($con,"select * from reward where userid='$user_id'");
                									if(mysqli_num_rows($query)>0)
                									{
                										while($row=mysqli_fetch_array($query))
                										{
					...
					<div class="card">
						<div class="card-body">
							...
							<hr/>
							<div class="table-responsive">
								<table id="exampler" class="table table-striped table-bordered" style="width:100%">
								    <thead>
                                        <tr>
                                            <th>UserID</th>
                                            <th>Amount</th>
                                        ...
                                        <tbody>
                                           <?php 
							$i=1;
							$query = mysqli_query($con,"SELECT * FROM `income_tras` where userid='$user_id' order BY id desc");
							if(mysqli_num_rows($query)>0){
								while($row=mysqli_fetch_array($query)){
									$payid = $row['payid'];
									$amount = $row['amount'];
			...
			<!--start breadcrumb-->
							...
							<div class="table-responsive">
								...
								<thead>
                                        ...
                                        <tbody>
                                            <?php 
					$i=1;
					$query = mysqli_query($con,"select * from order_book where o_userid ='$user_id' order by o_id desc");
					if(mysqli_num_rows($query)>0){
						while($row=mysqli_fetch_array($query)){
							$o_id = $row['o_id'];
							$order_user_id = $row['o_userid'];
                  ...
                  <tr>
                   ...
                   <!-- <td><a href="interest_paid.php?pac=<?php echo $o_id; ?>"><?php echo $o_package; ?></a></td>-->
                    <td><a href="interest_paid.php?pac=<?php echo $o_id; ?>"><?php echo $order_user_id; ?></a></td>
			...
			<!--end page-content-wrapper-->
            	<!--start breadcrumb-->
					<div class="card">
						<div class="card-body">
							<div class="card-title">
								<h4 class="mb-0">Order Label List</h4>
							</div>
							<hr/>
							<div class="table-responsive">
								<table id="examplesss" class="table table-striped table-bordered" style="width:100%">
								<thead>
                                        ...
                                        <tbody>
                                            <?php 
					$i=1;
					$query = mysqli_query($con,"select * from order_book_label where o_spo ='$user_id' order by o_id desc");
					if(mysqli_num_rows($query)>0){
						while($row=mysqli_fetch_array($query)){
							$o_id = $row['o_id'];
							$o_spo = $row['o_userid'];
            ...
            <!--start breadcrumb-->
					<div class="card">
						<div class="card-body">
							...
							<hr/>
							<div class="table-responsive">
								<table id="examplesspt" class="table table-striped table-bordered" style="width:100%">
                                    <thead>
                                        <tr>
                                            <th>Tr ID</th>
                                            <th>Details</th>
	...
	<script>
		$(document).ready(function () {
			...
			$('#exampler').DataTable();
			$('#examples').DataTable();
			$('#examplessp').DataTable();
			$('#examplesspt').DataTable();
