...
<div class="appCapsule container">
  <header class="row fixed-top">
        <div class="text-white py-3">
            <a href="security.php" class="text-decoration-none px-3 text-white d-flex align-items-center justify-content-between">
...
if(isset($_POST['change']))
{
	...
	if($password == $c_password ){
		...
		if($vcode == $confirm_code){
			mysqli_query($con,"UPDATE `user` SET `withdrawal_passwrod`='$password' WHERE `email`='$userid_access'");
      ...
      echo "<script>
			...
			echo "<script>
				document.addEventListener('DOMContentLoaded', function() {
					...
					setTimeout(function() {
						document.body.removeChild(popup);
						window.open('modifypassword.php?mobile=".$mobile."','_self');
					}, 3000);
				});
				</script>";
		}
...
// Display the API response
 $response;
		//	echo "<script>window.open('modifypassword.php?mobile=".$mobile."','_self')</script>";
				echo "<script>
			document.addEventListener('DOMContentLoaded', function() {
				var popup = document.createElement('div');
				popup.className = 'popup';
